...
% Sens de projection : vaut 1 si I2 se projette à droite de I1, 0 sinon 
% (on regarde seulement la projection de la coordonnée selon les colonnes 
% du coin en haut à gauche de I2)
sens_projection = xy_coinsI2_R1(1,1) > 0;
...
for x=1:nbcImos
  for y=1:nblImos
    ...
    if(x_R2>=1 && x_R2<=nbcI2 && y_R2>=1 && y_R2<=nblI2)
        ...
        % Traitement différent si xy est dans les 2 images
        if (x_R1>=1 && x_R1<=nbcI1 && y_R1>=1 && y_R1<=nblI1)
            % Traitement différent si I2 se projette à droite ou à gauche
            % de I1
            if sens_projection
                d1 = nbcI1 - x_R1;
                d2 = x_R2;
