...
function setUp()
  ...
  % file names
  javaapiname = 'ampl-java.jar';
  matlabbridgename = 'matlab-lib.jar'
  if ispc
    binlibraryname = fullfile('bin', 'ampl-2.3.13.dll');
    binbase = fullfile(base, '..', 'bin');
  elseif ismac
    binlibraryname = fullfile('lib', 'libampl.2.3.13.dylib');
    binbase = fullfile(base, '..', 'lib');
  else
    binlibraryname = fullfile('lib', 'libampl.so.2.3.13');
    binbase = fullfile(base, '..', 'lib');
  end
